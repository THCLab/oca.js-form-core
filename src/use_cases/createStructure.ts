...
export const createStructure = (oca: any): Structure => {
  ...
  const sectionsFromLabel = getSectionsFromLabel(filterOverlaysByType(oca.overlays, "label"))
  const attributesFromLabel = getAttributesFromLabel(filterOverlaysByType(oca.overlays, "label"))
  const attributesFromCharacterEncoding = getAttributesFromCharacterEncoding(filterOverlaysByType(oca.overlays, "character_encoding")[0])

  Object.entries(sectionsFromLabel).forEach(([id, { translations, attributes }]) => {
    ...
    attributes.forEach((attr_name: string) => {
      const data: ControlData = {
        ...
        characterEncoding: attributesFromCharacterEncoding.attributes[attr_name] || attributesFromCharacterEncoding.default,
        translations: {
...
const getSectionsFromLabel = (labelOverlays: any[]) => {
  ...
  labelOverlays.forEach(o => {
    o.attr_categories.forEach((cat: string) => result[cat] ||= { translations: {}, attributes: [] })
    Object.entries(o.cat_attributes).forEach(([cat, attributes]: [string, string[]]) => {
      const attrs = result[cat].attributes
      if (attrs.length === 0) { attrs.push(...attributes) }
    })

    Object.entries(o.cat_labels).forEach(([cat, label]: [string, string]) => result[cat].translations[o.language] = { label })
  })
  return result
}
...
const getAttributesFromLabel = (labelOverlays: any[]) => {
  ...
  labelOverlays.forEach(o => {
    Object.entries(o.attr_labels).forEach(([attr_name, label]: [string, string]) => {
...
const getAttributesFromCharacterEncoding = (encodingOverlay: any) => {
  ...
  Object.entries(encodingOverlay.attr_character_encoding).forEach(([attr_name, encoding]: [string, string]) => {
